...
{	
	...
	public function homePage() {
		...
		$sql_query = '
			...
			SELECT 
				...
				ANY_VALUE(place_reviews.place_id),
	...
	public function locationPage($state_id, $town_id, $category_id, $page = null) {
		...
		$limit_by_category = $category_id == null ? '' : 'and category_id = '.$category_id; 
		$cat = $category_id == null ? '' : 'places.category,'; 
		...
		$stmt = $this->db->prepare($sql_query); // stmt = countyment
		$stmt -> execute();
	...
	public function itemPage($id) {
		...
		$sql_query = '
			...
			SELECT
				...
				place_images.attribute_value as images
		...
		*/
		...
		if (count($res['comments'])<1 or strlen($t)<80)
		{
			...
			-- and 
				...
				and 
				places.id < '.($id + $last_digit + 200000).'
