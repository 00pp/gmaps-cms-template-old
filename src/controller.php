...
{	
	...
	{		 
		...
		$this->twig->display('home.html.twig', 
			[
				...
				// 'counties' => $model['state_id'],
				// 'cities' => $model['town_id'],
				'items' => $model['items'], 
	...
	{	
		...
		{	
			...
			foreach ($item['comments'] as $key => $value) {
				...
				$item['comments'][$key]['text'] = preg_replace('#^(\s+|)<br>#', '', $item['comments'][$key]['text']);
		...
		if (isset($item['timetable']) and empty($item['timetable'][0]['day']))
			$item['timetable'] = null;
	...
	public function locationPage($state_id, $town_id, $category_id, $page)
	{	

		if ($page == null)
			$page = 1;

		$model = $this -> db -> locationPage($state_id, $town_id, $category_id, $page);
		...
		$next_page_url = '/location/'.$state_id;
		$next_page_url .= $town_id !== '' ? '/'.$town_id : '';
		$next_page_url .= isset($category_id) ? '/category/'.$category_id : '';

		// echo $next_page_url;	

		$this->twig->display('location.html.twig', 
			[
				'location' => $item, 
